...
def setup_parser(parser: ArgumentParser):
  ...
  parser.add_argument("--type-hint-count", type=int, default=3)
...
def main(args):
  ...
  for e in range(args.epoch):
    ...
    with torch.no_grad():
      ...
      validation_result = learn.run(method, model, test_set, args, prompt=f"[Test] Epoch {e}")
    ...
    torch.save(model, f"{args.output}.epoch{e}.last.model")
