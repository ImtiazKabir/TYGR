...
if [ "$#" -ne 3 ]; then
    ...
    echo "Example: $0 /path/to/graphs /path/to/hypergraphs ./progress/hypergraph_O0.txt"
    echo ""
    echo "Options:"
...
for INPUT_PKL in "$INPUT_DIR"/*.pkl; do
    [ -f "$INPUT_PKL" ] || continue  # skip if not a file
    ...
    if ./TYGR hypergraph_convert "$INPUT_PKL" "$OUTPUT_PKL" $VERBOSE; then
